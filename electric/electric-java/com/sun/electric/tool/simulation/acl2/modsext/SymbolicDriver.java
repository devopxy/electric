...
public class SymbolicDriver<N extends SvarName>
{
    ...
    public Driver<N> makeWideDriver(Svar.Builder<N> builder)
    {
        Map<Svex<PathExt>, Svex<N>> memoize = new HashMap<>();
        return new Driver<>(subst(drv.getSvex(), memoize, builder), drv.getStrength());
    }

    public Driver<N> makeDriver(Svar.Builder<N> builder)
    {
        Map<Svex<PathExt>, Svex<N>> memoize = new HashMap<>();
        Svex<N> Z = SvexQuote.Z();
        Svex<N> svex = subst(drv.getSvex(), memoize, builder);
        svex = svex.rsh(rsh);
        svex = svex.concat(width, Z);
        svex = Z.concat(lsh, svex);
        return new Driver<>(svex, drv.getStrength());
    }

    private Svex<N> subst(Svex<PathExt> x, Map<Svex<PathExt>, Svex<N>> memoize, Svar.Builder<N> builder)
    {
        ...
        if (result == null)
        {
            if (x instanceof SvexVar)
            {
                ...
                result = arg.toSvex().addDelay(sv.svar.getDelay(), builder, addDelayCache);
            ...
            } else
            {
                ...
                {
                    newArgs[i] = subst(args[i], memoize, builder);
                }
                result = sc.fun.<N>callStar(newArgs);
            }
