...
public class Compile<N extends SvarName>
{
    ...
    Compile(List<Lhs<N>> aliases, Map<Lhs<IndexName>, Driver<IndexName>> assigns,
        Svar.Builder<N> builder)
    {
        this.aliases = aliases;
        svexarr = Svex.newSvexArray(aliases.size());
        for (int i = 0; i < aliases.size(); i++)
        {
            svexarr[i] = aliases.get(i).toSvex();
        }
    ...
    private Svex<N> svexSubstFromSvexarr(Svex<IndexName> x,
        Map<SvexCall<IndexName>, Svex<N>> memoize, Svar.Builder<N> builder)
    {
        if (x instanceof SvexVar)
        {
            ...
            return svex.addDelay(sv.svar.getDelay(), builder, addDelayCache);
        ...
        } else
        {
            ...
            if (newX == null)
            {
                ...
                {
                    newArgs[i] = svexSubstFromSvexarr(args[i], memoize, builder);
                }
                newX = sc.fun.<N>callStar(newArgs);
                memoize.put(sc, newX);
            }
            return newX;
        }
        ...
        Map<SvexCall<IndexName>, Svex<N>> memoize, Svar.Builder<N> builder)
    {
        ...
        for (Map.Entry<Lhs<IndexName>, Driver<IndexName>> e : assigns.entrySet())
        {
            ...
            Svex<N> val = svexSubstFromSvexarr(drv.svex, memoize, builder);
    ...
    private Map<Svar<N>, List<Driver<N>>> assignsToNetassigns(Map<Lhs<N>, Driver<N>> assigns)
    {
        ...
        for (int i = assignsEntries.size() - 1; i >= 0; i--)
        {
            ...
            for (int j = lhs.ranges.size() - 1; j >= 0; j--)
            {
                ...
                if (svar != null)
                {
                    Svex<N> svex = drv.svex.rsh(offset);
                    svex = svex.concat(range.getWidth(), Z);
                    svex = Z.concat(range.getRsh(), svex);
    ...
    private Map<Svar<N>, Svex<N>> netassignsResolves(Map<Svar<N>, List<Driver<N>>> netassigns)
    {
        ...
        for (Map.Entry<Svar<N>, List<Driver<N>>> e : netassigns.entrySet())
        {
            ...
            {
                svex = SvexCall.newCall(Vec4Res.FUNCTION, drivers.get(i).svex, svex);
            }
    ...
    public ACL2Object normAssignsAsACL2Object()
    {
        ...
        for (Map.Entry<Lhs<N>, Driver<N>> e : normAssigns.entrySet())
        {
            Lhs<N> lhs = e.getKey();
            Driver<N> drv = e.getValue();
            result = cons(cons(lhs.getACL2Object(), drv.getACL2Object()), result);
        }
